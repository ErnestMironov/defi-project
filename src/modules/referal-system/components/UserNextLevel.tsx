...
export default function UserNextLevel({
  ...
  return (
    <div className="relative">
      <div
        className="absolute left-0 top-0 z-10 h-full rounded-l-xl bg-[#8585A91F] transition-all duration-300"
        style={{
          width: `${(userRewards?.totalPoints ?? 0) / 10}%`,
        }}
      />
      <div
        className={cn(
          'relative flex w-full flex-col z-11 gap-3 rounded-xl border-[1.5px] border-stroke-100 bg-cards-widget p-6 pb-3 shadow-[0px_6px_36px_0px_rgba(0,0,0,0.09),_0px_6px_9px_0px_rgba(0,0,0,0.04)] max-md:border-none',
          className,
        )}
        {...props}
      >
        ...
        <div className="max-md:flex-start flex items-center gap-2 max-md:flex-wrap max-md:items-start max-md:justify-start">
          ...
          <StarPoint value={nextLevel?.nextLvlBenefits.rewardsPerActivity ?? 0} />
