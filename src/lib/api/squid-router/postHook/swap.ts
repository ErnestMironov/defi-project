import { swapRouterAbi } from '@constants/abi/swap-router'
...
export async function swapExactOutputSingle(
  ...
  const quotedAmountIn: bigint = await quoterContract.quoteExactOutputSingle.staticCall(
  ...
  const swapRouter = new Contract(SWAP_ROUTER_ADDRESS, swapRouterAbi, arbitrumProvider)
  ...
  const tx = await swapRouter.exactOutputSingle.populateTransaction(parameters)
